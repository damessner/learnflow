...
router.post('/', requireAuth, requireRole('teacher', 'admin'), validate(createWorksheetSchema), async (req, res, next) => {
  try {
    const knex = getKnex()
    const id = uuidv4()
...
router.put('/:id', requireAuth, requireRole('teacher', 'admin'), async (req, res, next) => {
  try {
    ...
    const fields = ['title', 'description', 'subject', 'grade_level', 'content', 'total_points', 'is_published', 'tags', 'rubric_json', 'in_library']
...
router.post('/:id/duplicate', requireAuth, requireRole('teacher', 'admin'), async (req, res, next) => {
  try {
    const knex = getKnex()
...
router.post('/templates/:id/clone', requireAuth, requireRole('teacher', 'admin'), async (req, res, next) => {
  try {
    const knex = getKnex()
    const id = uuidv4()
    const templateData = { title: `Cloned Template ${req.params.id}`, content: JSON.stringify({ blocks: [] }) }
...
router.delete('/:id/assignments/:assignmentId', requireAuth, requireRole('teacher', 'admin'), async (req, res, next) => {
  try {
    const knex = getKnex()
...
router.get('/assignments/:assignmentId/results', requireAuth, requireRole('teacher', 'admin'), async (req, res, next) => {
  try {
    const knex = getKnex()
...
router.get('/assignments/:assignmentId/stats', requireAuth, requireRole('teacher', 'admin'), async (req, res, next) => {
  try {
    const knex = getKnex()
    const submissions = await knex('submissions')
...
router.post('/ai/generate', requireAuth, requireRole('teacher', 'admin'), async (req, res, next) => {
  try {
...
router.post('/ai/neuro-vocab', requireAuth, requireRole('teacher', 'admin'), async (req, res, next) => {
  try {
    const { rawList } = req.body
    const words = (rawList || '').split(/[\n,]+/).filter(Boolean).map((w: string) => w.trim())
