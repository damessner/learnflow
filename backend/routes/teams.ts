...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const teams: { id: string; name: string; members: string[] }[] = Array.from(
        { length: count },
        (_, i) => ({
          id: uuidv4(),
          name: `Team ${i + 1}`,
    ...
    try {
      ...
      for (const team of teams) {
        ...
        const memberScores = await knex('submissions')
          .whereIn(
            'user_id',
            members.map((m: { student_id: string }) => m.student_id),
          )
