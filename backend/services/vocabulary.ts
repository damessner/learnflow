...
export async function addVocabularyItem(
  userId: string,
  item: { source_lang: string; target_lang: string; word: string; translation: string; base_form?: string; part_of_speech?: string; context_sentence?: string; kc_id?: string },
...
export async function getVocabularyItems(
  userId: string,
  filters?: { source_lang?: string; target_lang?: string; min_mastery?: number; max_mastery?: number; part_of_speech?: string },
): Promise<VocabularyItem[]> {
  const knex = getKnex()
  ...
  if (filters?.min_mastery !== undefined) query = query.andWhere('mastery_level', '>=', filters.min_mastery)
  if (filters?.max_mastery !== undefined) query = query.andWhere('mastery_level', '<=', filters.max_mastery)
  if (filters?.part_of_speech) query = query.andWhere({ part_of_speech: filters.part_of_speech })
  
  return query.orderBy('mastery_level', 'asc').orderBy('last_reviewed_at', 'asc') as unknown as VocabularyItem[]
}
...
export async function updateVocabularyMastery(
  ...
  await knex('vocabulary_items').where({ id: wordId }).update({
...
export async function getVocabularyStats(userId: string): Promise<{
  ...
  const learning = items.filter((i: { mastery_level: number }) => i.mastery_level > 0 && i.mastery_level < 80).length
