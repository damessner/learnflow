<template>
  <div>
    ...
    >
      ...
      ></div>
      <div
        v-for="(pt, i) in block.points_to_plot || []"
        :key="'e' + i"
        :style="{
          position: 'absolute',
          left: calcPos(pt[0], 0) + '%',
    ...
    <button
      v-if="!readonly && userPoints.length"
      class="btn-sm"
...
function plotPoint(e) {
  ...
  emit(
    'update:modelValue',
    userPoints.value.map((p) => [Math.round(p.x), Math.round(p.y)]),
  )
}
