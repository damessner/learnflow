<template>
  <div>
    <div v-for="(pair, pi) in (block.pairs || [])" :key="pi" style="display:flex;gap:0.5rem;align-items:center;margin-bottom:0.25rem">
      ...
      <input v-if="!readonly" :value="(modelValue || {})[pi] || ''" @input="update(pi, $event.target.value)" style="flex:1" />
      <span v-else style="flex:1;color:var(--text-muted)">{{ (modelValue || {})[pi] || '___' }}</span>
    </div>
  </div>
</template>
