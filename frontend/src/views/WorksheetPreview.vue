  ...
  <div class="page">
    ...
    >
      ...
      <button
        class="btn btn-lg"
        :style="{
          background: showAnswers ? 'var(--warning)' : 'var(--bg-card)',
          color: showAnswers ? '#000' : 'var(--text-main)',
        }"
      ...
      <button
        ...
        class="btn btn-lg"
        :style="{
          background: checked ? 'var(--success)' : 'var(--bg-card)',
    ...
    <div v-for="(block, bIdx) in blocks" :key="block.id" class="card" style="margin-bottom: 0.5rem">
      <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem">
        ...
        <span
          v-if="checked && blockResults[bIdx] !== undefined"
          :style="{
            color: blockResults[bIdx] ? 'var(--success)' : 'var(--danger)',
            fontWeight: 600,
            fontSize: '0.85rem',
          }"
        >
            ...
            <span v-else-if="seg.type === 'gap'">
              <input
                ...
                :style="{
                  ...
                  border:
                    checked &&
                    answers[block.id][seg.key]?.toLowerCase().trim() === seg.answer.toLowerCase()
              ...
              />
              <span v-else :style="{ color: 'var(--success)', fontWeight: 600 }">{{
                seg.answer
              }}</span>
            </span>
          </template>
      ...
      <template v-if="block.type === 'multiple_choice'">
        <div
          ...
          :key="oi"
          style="padding: 0.25rem 0; display: flex; align-items: center; gap: 0.5rem"
        >
          <input
            v-if="!showAnswers"
            type="checkbox"
          ...
          />
          <span
            v-if="showAnswers && block.correct && block.correct.includes(oi)"
        ...
        >
          ...
          <input
            ...
            :style="{
              flex: 1,
              border:
                checked && answers[block.id][pi]?.toLowerCase().trim() === pair[1].toLowerCase()
                  ? '2px solid var(--success)'
                  : checked
                    ? '2px solid var(--danger)'
      ...
      <template v-if="block.type === 'short_answer'">
        ...
        <textarea
          ...
          :disabled="checked"
        ></textarea>
        <div v-else style="color: var(--success); font-weight: 600; margin-top: 0.25rem">
          {{ block.sample_answer || '(Sample answer)' }}
        </div>
      </template>
      ...
      <template v-if="block.type === 'true_false'">
        <div style="margin-top: 0.25rem; display: flex; gap: 1rem; align-items: center">
          ...
          <span
            v-if="checked && answers[block.id] !== undefined"
            :style="{
              color:
      ...
      <template v-if="block.type === 'ordering'">
        <div v-if="!showAnswers" style="margin-top: 0.25rem">
          ...
          <div
            ...
            "
          >
            <span style="color: var(--text-muted); font-size: 0.8rem; min-width: 24px"
              >#{{ oi + 1 }}</span
            >
            <button
              class="btn-sm"
              style="padding: 0.1rem 0.3rem; font-size: 0.7rem"
              @click="moveOrderItem(block.id, oi, -1)"
              :disabled="oi === 0"
            >
              ▲
            </button>
            <button
              ...
              style="padding: 0.1rem 0.3rem; font-size: 0.7rem"
              @click="moveOrderItem(block.id, oi, 1)"
              :disabled="oi === orderingItems(block).length - 1"
            >
              ▼
            </button>
            <span>{{ item }}</span>
        ...
        <div>
          <template v-for="(seg, si) in getCorrectWordsSegments(block.template)" :key="si">
            ...
            <span v-else-if="seg.type === 'word'">
              <span v-if="!showAnswers">
                <input
                  ...
                  :style="{
                    ...
                    border: checked
                      ? answers[block.id][String(seg.index)]?.toLowerCase().trim() ===
                        seg.correct.toLowerCase()
                        ? '2px solid var(--success)'
                        : '2px solid var(--danger)'
                      : '1px dashed var(--primary)',
                    borderRadius: '4px',
              ...
              <span v-if="showAnswers" style="color: var(--text-muted); font-size: 0.75rem">
                (was {{ seg.wrong }})</span
              >
            </span>
          </template>
        ...
        <div>
          <template v-for="(seg, si) in getCorrectWordsSegments(block.template)" :key="si">
            ...
            <span v-else-if="seg.type === 'word'" style="color: var(--text-muted)">
              <del style="color: red">{{ seg.wrong }}</del> (<ins
          ...
          <thead>
            <tr style="border-bottom: 1px solid var(--border-color); background: var(--bg-main)">
              ...
              <th
                ...
                :key="col"
                style="text-align: center; padding: 0.25rem 0.5rem; width: 80px"
              >
                {{ col }}
              </th>
            </tr>
        ...
        <div v-if="block.words?.length" style="display: flex; gap: 1.5rem; flex-wrap: wrap">
          <div>
            <div
              ...
              :key="idx"
              style="margin-bottom: 0.35rem; font-size: 0.85rem"
            >
        ...
        >
          ...
          <input
            ...
            :style="{
              flex: 1,
              border:
                checked && answers[block.id][wi]?.toLowerCase().trim() === w.word.toLowerCase()
                  ? '2px solid var(--success)'
                  : checked
                    ? '2px solid var(--danger)'
                    : '1px solid var(--border-color)',
      ...
      <template v-if="block.type === 'dictation'">
        <div>
          <button class="btn-sm" @click="playAudio(block.audioText)" style="margin-bottom: 0.5rem">
            🔊 Play Audio
          </button>
          <textarea
            v-if="!showAnswers"
        ...
        <div v-if="!showAnswers" style="margin-top: 0.25rem">
          ...
          <div style="display: flex; flex-wrap: wrap; gap: 0.35rem; margin-bottom: 0.5rem">
            <span
              ...
              :style="{
                padding: '0.25rem 0.5rem',
                background: selectedSentenceWords(block.id).includes(word)
                  ? 'var(--primary)'
                  : 'var(--bg-main)',
          ...
          <div
            v-if="selectedSentenceWords(block.id).length > 0"
            style="
              padding: 0.5rem;
              background: var(--bg-card);
              border-radius: 4px;
          ...
          <span
            ...
            :style="{
              color:
                selectedSentenceWords(block.id).join(' ').toLowerCase() ===
                block.sentence.toLowerCase()
                  ? 'var(--success)'
                  : 'var(--danger)',
              marginLeft: '0.5rem',
              fontWeight: 600,
...
const totalPoints = computed(() =>
  blocks.value.reduce((s: number, b: Record<string, any>) => s + (b.points || 0), 0),
)
...
function gradeBlock(block: Record<string, any>): { correct: boolean; earned: number } {
  ...
  switch (block.type) {
    ...
    case 'multiple_choice': {
      ...
      const isCorrect =
        correctIndices.length === userSel.length && correctIndices.every((i) => userSel.includes(i))
    ...
    case 'true_false': {
      ...
      return {
        ...
        earned: userAnswer === correctAnswer ? pts : 0,
      }
    }
    case 'matching': {
      ...
      for (let i = 0; i < pairs.length; i++) {
        if ((ans?.[i] || '').toLowerCase().trim() === (pairs[i][1] || '').toLowerCase().trim())
          correct++
      }
    ...
    case 'short_answer': {
      ...
      return {
        correct: hits >= Math.ceil(keywords.length * 0.6),
        earned: keywords.length > 0 ? Math.round((hits / keywords.length) * pts) : 0,
      }
    }
    case 'word_scramble': {
      ...
      for (let i = 0; i < words.length; i++) {
        if (
          (ans?.[i] || '').toLowerCase().trim() === (words[i].word || words[i]).toLowerCase().trim()
        )
          correct++
      }
    ...
    case 'ordering': {
      ...
      const allAscending = userOrder.every(
        (_, i) => i === 0 || Number(userOrder[i]) >= Number(userOrder[i - 1]),
      )
    ...
    case 'audio_match': {
      ...
      if (block.type === 'dictation') {
        const isCorrect =
          (ans || '').toLowerCase().trim() === (block.audioText || '').toLowerCase().trim()
        return { correct: isCorrect, earned: isCorrect ? pts : 0 }
      }
...
function parseGapPreviewTemplate(
  template: string,
): Array<
  { type: 'text'; text: string; key: string } | { type: 'gap'; answer: string; key: string }
> {
