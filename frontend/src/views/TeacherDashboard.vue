<template>
  <div class="page">
    ...
    <div style="display:flex;gap:0.5rem;margin:1rem 0">
      <button :class="{ 'btn-primary': tab === 'worksheets' }" @click="tab = 'worksheets'">Worksheets</button>
      <button :class="{ 'btn-primary': tab === 'classes' }" @click="tab = 'classes'">Classes</button>
      <button :class="{ 'btn-primary': tab === 'results' }" @click="tab = 'results'">Results</button>
      <button :class="{ 'btn-primary': tab === 'analytics' }" @click="tab = 'analytics'">Analytics</button>
    </div>

    <template v-if="tab === 'worksheets'">
      <div v-if="wsStore.worksheets.length === 0" style="color:var(--text-muted)">No worksheets yet</div>
      <div v-for="ws in wsStore.worksheets" :key="ws.id" class="card" style="margin-bottom:0.5rem">
        ...
        <div v-if="assignTarget && assignTarget.id === ws.id" style="margin-top:0.5rem;padding-top:0.5rem;border-top:1px solid var(--border-color)">
      ...
      <div v-for="c in classesList" :key="c.id" class="card" style="margin-bottom:0.5rem">
        ...
        <div v-if="selectedClass === c.id" style="margin-top:0.5rem">
          <div v-if="classProgress.length">
            <div v-for="p in classProgress" :key="p.student.id" style="display:flex;justify-content:space-between;padding:0.25rem 0">
    ...
    <template v-if="tab === 'results'">
      <div v-if="!selectedAssignment" style="color:var(--text-muted)">Select a worksheet and assignment first</div>
      <div v-else>
        <h3>Results for {{ selectedAssignment.worksheet_title }}</h3>
        <table v-if="results.length">
          <thead><tr><th>Student</th><th>Score</th><th>Submitted</th><th>Feedback</th></tr></thead>
          <tbody>
    ...
    <template v-if="tab === 'analytics'">
      <div class="grid grid-3">
        <div class="card" v-if="learningStore.analytics">
          <h4>Classes</h4>
          <p style="font-size:2rem;font-weight:700">{{ learningStore.analytics.totalClasses }}</p>
        </div>
        <div class="card" v-if="learningStore.analytics">
          <h4>Assignments</h4>
          <p style="font-size:2rem;font-weight:700">{{ learningStore.analytics.totalAssignments }}</p>
        </div>
        <div class="card" v-if="learningStore.analytics">
          <h4>Avg Score</h4>
          <p style="font-size:2rem;font-weight:700">{{ learningStore.analytics.averageScore }}%</p>
        </div>
      </div>

      <div class="card" style="margin-top:1rem">
        <h4>At-Risk Students</h4>
        <div v-if="learningStore.atRisk.length === 0" style="color:var(--text-muted)">No at-risk students</div>
        <div v-for="s in learningStore.atRisk" :key="s.id" style="display:flex;justify-content:space-between;padding:0.25rem 0">
          <span>{{ s.name }}</span>
          <span style="color:var(--danger)">{{ s.completed }}/{{ s.total }} completed ({{ s.avgScore }}%)</span>
        </div>
      </div>
    </template>
  </div>
...
function editWs(id) { router.push(`/teacher/builder/${id}`) }
function previewWs(id) { router.push(`/teacher/preview/${id}`) }

async function duplicateWs(id) {
  try { await wsStore.duplicateWorksheet(id); await wsStore.fetchMyWorksheets(); uiStore.showToast('Duplicated', 'success') }
  catch (e) { uiStore.showToast(e.message, 'error') }
}

async function deleteWs(id) {
  try { await wsStore.deleteWorksheet(id); await wsStore.fetchMyWorksheets(); uiStore.showToast('Deleted', 'success') }
  catch (e) { uiStore.showToast(e.message, 'error') }
}

function assignWs(ws) { assignTarget.value = ws }
...
async function deleteClass(id) {
  try { await classesStore.deleteClass(id); await classesStore.fetchClasses(); classesList.value = classesStore.classes }
  catch (e) { uiStore.showToast(e.message, 'error') }
}

async function viewClassProgress(id) {
  selectedClass.value = id
  try { const data = await classesStore.fetchClassProgress(id); classProgress.value = data.progress }
  catch { /* */ }
}

async function exportCsv(id) {
  try { await classesStore.exportCsv(id); uiStore.showToast('CSV downloaded', 'success') }
  catch { /* */ }
}

function giveFeedback(r) { feedbackTarget.value = r; feedbackText.value = r.feedback || '' }
