<template>
  <div class="page">
    ...
    <div v-if="blocks.length === 0" class="card" style="text-align:center;color:var(--text-muted);padding:2rem">
    ...
    <div v-for="(block, idx) in blocks" :key="block.id" class="card" style="margin-bottom:0.5rem">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.5rem">
        ...
        <div style="display:flex;gap:0.25rem">
          ...
          <button class="btn-sm" @click="moveBlock(idx, -1)" :disabled="idx === 0">Up</button>
          <button class="btn-sm" @click="moveBlock(idx, 1)" :disabled="idx === blocks.length - 1">Down</button>
          <button class="btn-sm btn-danger" @click="removeBlock(idx)">Remove</button>
        </div>
      </div>

      <template v-if="block.type === 'gap_fill'">
        <textarea v-model="block.template" rows="3" placeholder="Text with ((answer)) placeholders"></textarea>
      </template>

      <template v-if="block.type === 'multiple_choice' || block.type === 'single_choice'">
        <div v-for="(opt, oi) in (block.options || [])" :key="oi" style="display:flex;gap:0.5rem;margin-bottom:0.25rem">
          <input v-model="block.options[oi]" :placeholder="`Option ${oi + 1}`" />
          <label v-if="block.type === 'multiple_choice'">
            <input type="checkbox" :value="oi" v-model="block.correct" /> Correct
          </label>
          <label v-else>
            <input type="radio" :value="oi" v-model="block.correct" /> Correct
          </label>
          <button class="btn-sm btn-danger" @click="block.options.splice(oi, 1)">X</button>
        </div>
        <button class="btn-sm" @click="block.options = [...(block.options || []), '']">Add Option</button>
      </template>
      ...
      <template v-if="block.type === 'matching'">
        ...
        <button class="btn-sm" @click="block.pairs = [...(block.pairs || []), ['', '']]">Add Pair</button>
      </template>
    ...
    <div class="card" style="margin-top:1rem">
      <h4>AI Generation</h4>
      <div style="display:flex;gap:0.5rem">
        <textarea v-model="aiPrompt" rows="2" placeholder="Describe the worksheet you want..." style="flex:1"></textarea>
        <select v-model="aiProvider" style="width:120px">
          <option value="ollama">Ollama</option>
          <option value="gemini">Gemini</option>
        </select>
        <button class="btn-primary" @click="generateAI" :disabled="aiLoading">Generate</button>
      </div>
    </div>
  </div>
</template>
...
function genId() { return crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).substring(2) }

function addBlock(type) {
  const block = { id: genId(), type, points: 10 }
  if (type === 'gap_fill') block.template = ''
  if (type === 'multiple_choice' || type === 'single_choice') { block.options = ['', '', '']; block.correct = type === 'single_choice' ? 0 : [] }
  if (type === 'matching') block.pairs = [['', ''], ['', '']]
  if (type === 'short_answer') { block.sample_answer = ''; block.keywordsStr = '' }
