...
- Retry logic (line 825) repeats the _same_ provider 3 times — if Gemini is down, all 3 attempts hit the same dead endpoint.
...
Centralize provider logic into a `ProviderService` that can be configured with a priority-ordered list of providers. Each provider has a `isAvailable()` check and a `generate()` method. The retry loop tries the _next available_ provider on each attempt, not the same one.
